...
async function loadPublicStats() {
  const el = document.getElementById('stat-residents-count');
  if (!el || !window.dbFetch) return;
  try {
    const residents = await dbFetch('residents');
    const count = residents?.length || 0;
    el.textContent = count >= 1000 ? `${Math.floor(count / 1000)}k+` : count.toLocaleString();
  } catch (_) {
    el.textContent = '—';
  }
}
...
// ===================== REALTIME — PUBLIC PORTAL (#031/#032/#033) =====================
// Subscribe to live changes so the public portal updates without a page refresh.
// supabase-config.js exposes dbSubscribe() globally.

function initPublicRealtime() {
  if (!window.supabaseClient) return; // offline — skip

  // Announcements: reload the swiper when any announcement changes
  supabaseClient
  ...
  // Projects: reload the swiper when any project changes
  supabaseClient
  ...
  // Suggestions: reload the voice board when a new reply is published
  supabaseClient
    .channel('public:suggestions')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'suggestions' }, (payload) => {
      // Only reload if a suggestion was published — avoid re-render on every pending insert
      if (!payload.new || payload.new.status === 'published' || payload.eventType === 'DELETE') {
        loadSuggestions();
      }
    })
    .subscribe();

  // Polls: reload poll cards when votes or status change
  supabaseClient
  ...
  console.log('✅ Public portal realtime active');
}
