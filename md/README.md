...
- **Document Requests** - Process and track official document certifications with **Admin Remarks**
- **Public Verification** - Resident-facing portal to verify documents with real-time status updates
...
### Recent Fixes
- **Login Query Formatting**: Fixed a critical issue where the authentication service would crash on emails/usernames containing special characters. The `sbAuthenticateUser` query string now properly double-quotes string literals inside the postgREST `.or()` filter.
...
- **CSS3** - Custom styling with CSS variables & Modern Layouts (Flexbox/Grid)
- **JavaScript** - Vanilla JS (ES6+) for application logic
- **Supabase** - Backend-as-a-Service (PostgreSQL database & real-time)
- **Google Fonts** - Public Sans (Headings) & Inter (Body)
- **Material Symbols** - Modern icon library
- **Aesthetics** - Glassmorphism, Fixed Backgrounds, and Slide-out Drawer Navigation
...
```
brgyorg-ph/
├── index.html              # Public Verification & Landing Page
├── admin.html              # Administrative Dashboard
├── css/
│   ├── landing.css         # Public portal styles
│   └── styles.css          # Admin portal styles
├── js/
│   ├── landing.js          # Public portal logic
│   └── app.js              # Administrative logic
├── md/
│   ├── README.md           # This file
│   └── DESIGN.md           # System design documentation
├── img/                    # Asset directory (logos, backgrounds)
